<?php
	...
	*/
				...
				'options' => array(
					'header-social-type' => array(
						...
						'options' => array(
							...
							'fa' => __('Font Awesome (Icon)', 'gdlr_translate')
						),
						'default' => 'dark'
					),
				)
	...
	if( !function_exists('gdlr_get_fa_social_icon') ){
		function gdlr_get_fa_social_icon( $slug ){
			...
			$icon = array(
				...
				'rss' 			=> 'fa fa-rss',
				'skype'			=> 'fa fa-skype',
				'stumble-upon' 	=> 'fa fa-stumbleupon',
				'tumblr' 		=> 'fa fa-tumblr',
...
?>
<div class="social-icon gdlr-type-<?php echo esc_attr($type); ?>">
<a href="<?php echo esc_url($theme_option[$social_slug . '-header-social']); ?>" target="_blank" >
<?php if( $type == 'fa' ){ ?>
	<i class="<?php echo esc_attr(gdlr_get_fa_social_icon($social_slug)); ?>" ></i>
<?php }else{ ?>
...
<?php				
				...
				'options' => array(
					'enable-social-share'=> array(
						...
						'options' => array(
							...
							'enable' => __('Image Icon', 'gdlr_translate'),
							'fa' => __('Font Awesome Icon', 'gdlr_translate'),
						),
						'default' => 'enable',
	...
	if( !function_exists('gdlr_get_social_shares') ){
		function gdlr_get_social_shares(){	
			...
			$type = empty($theme_option['enable-social-share'])? 'enable': $theme_option['enable-social-share'];
			$page_title = get_the_title();
			$current_url = GDLR_HTTP . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];

			if($theme_option['enable-social-share'] != 'disable'){ ?>
<div class="gdlr-social-share gdlr-type-<?php echo esc_attr($type); ?>">
...
<?php if($theme_option['digg-share'] == 'enable'){ ?>
	<a href="http://digg.com/submit?url=<?php echo esc_url($current_url); ?>&#038;title=<?php echo rawurlencode($page_title); ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('digg')); ?>" ></i>
		<?php }else{ ?>
...
<?php if($theme_option['facebook-share'] == 'enable'){ ?>
	<a href="http://www.facebook.com/share.php?u=<?php echo esc_url($current_url); ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('facebook')); ?>" ></i>
		<?php }else{ ?>
...
<?php if($theme_option['google-plus-share'] == 'enable'){ ?>
	<a href="https://plus.google.com/share?url=<?php echo esc_url($current_url); ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=400,width=500');return false;">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('google-plus')); ?>" ></i>
		<?php }else{ ?>
...
<?php if($theme_option['linkedin-share'] == 'enable'){ ?>
	<a href="http://www.linkedin.com/shareArticle?mini=true&#038;url=<?php echo esc_url($current_url); ?>&#038;title=<?php echo esc_url($current_url); ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('linkedin')); ?>" ></i>
		<?php }else{ ?>
...
<?php if(false && $theme_option['my-space-share'] == 'enable'){ ?>
	<a href="http://www.myspace.com/Modules/PostTo/Pages/?u=<?php echo esc_url($current_url); ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('my-space')); ?>" ></i>
		<?php }else{ ?>
...
?>
	<a href="http://pinterest.com/pin/create/button/?url=<?php echo esc_url($current_url); ?>&media=<?php echo esc_url($thumbnail[0]); ?>" class="pin-it-button" count-layout="horizontal" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('pinterest')); ?>" ></i>
		<?php }else{ ?>
...
<?php if($theme_option['stumble-upon-share'] == 'enable'){ ?>
	<a href="http://www.stumbleupon.com/submit?url=<?php echo esc_url($current_url); ?>&#038;title=<?php echo rawurlencode($page_title); ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('stumble-upon')); ?>" ></i>
		<?php }else{ ?>
...
<?php if($theme_option['twitter-share'] == 'enable'){ ?>
	<a href="http://twitter.com/home?status=<?php echo str_replace('%26%23038%3B', '%26', $page_title) . ' - ' . $current_url; ?>" target="_blank">
		<?php if( $type == 'fa' ){ ?>
			<i class="<?php echo esc_attr(gdlr_get_fa_social_icon('twitter')); ?>" ></i>
		<?php }else{ ?>
