	...
	*/
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			$ret .= '<div class="column-service-divider"></div>';
	...
	if( !function_exists('gdlr_get_carousel_testimonial_item') ){
		function gdlr_get_carousel_testimonial_item( $settings ){	
			...
			if( !empty($settings['carousel-navigation']) && $settings['carousel-navigation'] == 'bottom' ){
				$ret .= '<div class="gdlr-testimonial-bottom-nav gdlr-nav-container" >';
				$ret .= '<i class="icon-angle-left gdlr-flex-prev"></i>';
				$ret .= '<span class="gdlr-item-separator" ></span>';
				$ret .= '<i class="icon-angle-right gdlr-flex-next"></i>';
				$ret .= '</div>';
			}
	...
	if( !function_exists('gdlr_get_page_list_item') ){
		function gdlr_get_page_list_item( $settings ){	
			...
			}else if($settings['page-style'] == 'hotel'){
				$ret .= gdlr_get_hotel_page_list($query, $settings['item-size'], 
							$settings['thumbnail-size'], $settings['page-layout'] );
	...
	// print hotel page list
	if( !function_exists('gdlr_get_hotel_page_list') ){
		function gdlr_get_hotel_page_list($query, $size, $thumbnail_size, $layout = 'fitRows'){
			$current_size = 0;
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<a href="' . get_permalink() . '" ><i class="fa fa-link icon-link"></i></a>';
				$ret .= '</div>';
				$ret .= '</div>';
 
				$ret .= '<h3 class="gdlr-hotel-branches-title" ><a href="' . get_permalink() . '">' . get_the_title() . '</a></h3>';
				$ret .= '</div>';				
				$ret .= '</div>';
				$current_size ++;
			}
	...
	if( !function_exists('gdlr_get_classic_page_list') ){
		function gdlr_get_classic_page_list($query, $size, $thumbnail_size, $layout = 'fitRows'){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<span class="portfolio-icon" ><i class="fa fa-link icon-link" ></i></span>';
				$ret .= '</a>';
