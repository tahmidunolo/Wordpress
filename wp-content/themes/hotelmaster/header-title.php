...
?>
	<?php  if(( !empty($hotel_option['booking-slug']) && isset($_GET[$hotel_option['booking-slug']]) ) || 
		...
		<div class="gdlr-page-title-wrapper" <?php echo empty($header_background)? '': 'style="background-image: url(\'' . esc_url($header_background) . '\'); "'; ?> >
			...
			<div class="gdlr-page-title-container container" >
				...
				<?php
					if( !empty($theme_option['page-title-divider']) && $theme_option['page-title-divider'] == 'vertical' ){
						echo '<span class="gdlr-page-title-vertical-divider" ></span>';
					}
				?>
			</div>	
		</div>			
	...
	<?php $page_background = ''; ?>
		<div class="gdlr-page-title-wrapper" <?php 
			...
			if( !empty($header_background) ){
				$css .= 'background-image: url(\'' . esc_url($header_background) . '\');';
			}
			if( !empty($gdlr_post_option['page-title-top-padding']) ){
				$css .= 'padding-top: ' . gdlr_check_option_data_type($gdlr_post_option['page-title-top-padding'], 'pixel') . ';';
			}
			if( !empty($gdlr_post_option['page-title-bottom-padding']) ){
				$css .= 'padding-bottom: ' . gdlr_check_option_data_type($gdlr_post_option['page-title-bottom-padding'], 'pixel') . ';';
			}
			if( !empty($css) ){
				echo 'style="' . $css . '"';
			}
	...
	<?php }else if( is_single() && $post->post_type == 'post' ){ 
		...
		if( !empty($theme_option['single-blog-style']) && $theme_option['single-blog-style'] == 'v4' ){
			...
			$page_caption = gdlr_get_blog_info(array('date', 'author', 'comment', 'category'));;
	...
	<?php }else if( is_single() && in_array($post->post_type, array('room', 'hostel_room')) ){ 
