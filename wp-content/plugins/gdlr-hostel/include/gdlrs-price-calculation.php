<?php
	...
	*/	
	...
	if( !function_exists('gdlrs_calculate_room_service_price') ){
		function gdlrs_calculate_room_service_price($data){
			$service_prices = array( 'total'=>0 );
			...
			foreach( $data['service-room'] as $key => $room ){
				...
				if( $option['service-type'] == 'parking-service' ){
					...
					if( $option['car'] == 'car' ){
						$car_amount = empty($data['service-room-amount'][$key])? 1: $data['service-room-amount'][$key];
						...
						if( !empty($option['car-unit']) ){
							...
							$service_prices[$room][$service]['title'] .= __('cars', 'gdlr-hostel');
						}
					...
					// count number of guest in the room
					$amount = 0;
					...
					if( $option['per'] == 'guest' ){
						$service_prices[$room][$service]['title'] .= '<span class="gdlr-sep">/</span>' . $amount . ' ' . __('guests', 'gdlr-hostel') ;
						$service_prices[$room][$service]['price'] *= $amount;
					}else if( $option['per'] == 'room' ){
						$service_prices[$room][$service]['title'] .= '<span class="gdlr-sep">/</span>' . $amount . ' ' . __('room', 'gdlr-hostel');
						$service_prices[$room][$service]['price'] *= $amount;
					}
				}
	...
	if( !function_exists('get_hostel_price_breakdown_popup') ){
		function get_hostel_price_breakdown_popup($post_option){
			...
			if( !empty($price['cnd']) ){		
				...
				$ret .= '</span>';
				$ret .= '<span class="gdlr-tail">-' . gdlr_hostel_money_format($price['cnd']) . '</span>';
				$ret .= '</div>';		
			}		
