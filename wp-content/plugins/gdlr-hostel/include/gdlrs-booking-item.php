...
<?php
	...
	if( !function_exists('gdlr_ajax_hostel_booking') ){
		function gdlr_ajax_hostel_booking(){	
			...
			if( !empty($_POST['service']) ){
				...
				$data['service-room'] = empty($service['service-room'])? array(): $service['service-room'];
				$data['service-room-select'] = empty($service['service-room-select'])? array(): $service['service-room-select'];
				$data['service-room-amount'] = empty($service['service-room-amount'])? array(): $service['service-room-amount'];
			}else if( empty($data['service']) ){
				$data['service'] = array();
				$data['service-amount'] = array();
				$data['service-room'] = array();
				$data['service-room-select'] = array();
				$data['service-room-amount'] = array();
			}
			$ret = array();
			...
			if( $_POST['state'] == 2 ){
				...
				if( empty($data['gdlr-check-in']) || empty($data['gdlr-check-out']) || $data['gdlr-check-out'] < $data['gdlr-check-in'] ){
					...
					$ret['content'] .= gdlrs_get_booking_services($data['gdlr-hotel-branches'], $data['service'], $data['gdlr-room-id']);
	...
	if( !function_exists('gdlrs_get_booking_room_query') ){
		function gdlrs_get_booking_room_query($data, $room_number){
			...
			if( $data['gdlr-night'] < $minimum_night ){
				...
				$ret .= sprintf(__('A minimum stay of %d consecutive nights is required', 'gdlr-hostel'), $minimum_night);
				$ret .= '</div>';
				return $ret;
			}if( $data['gdlr-check-in'] == $data['gdlr-check-out'] ){
				$ret  = '<div class="gdlr-hotel-missing-room">';
				$ret .= '<i class="fa fa-frown-o icon-frown"></i>';
				$ret .= __('Invalid check out date, please enter correct value to proceed', 'gdlr-hostel');
				$ret .= '</div>';
				return $ret;
			}			
	...
	if( !function_exists('gdlrs_get_booking_services') ){
		function gdlrs_get_booking_services($branches, $selected_service, $rooms = array()){
			...
			foreach( $rooms as $room_id ){
				if( empty($room_services[$room_id]) ){
					
					// find service for each room
					$args['post__not_in'] = $services;
					$args['meta_query'] = array(
						array(
							'key' => 'gdlr-hostel-room-id',
							'value' => $room_id,
							'compare' => 'LIKE'
						)
					);
					...
					$temp_service = array();
					foreach( $query->posts as $post ){
						$temp_service[] = $post->ID;
					}

					if( !empty($temp_service) ){
						$room_services[$room_id] = $temp_service;
					}
				}
			}

			if( empty($services) && empty($room_services) ) return;
...
<div class="gdlr-booking-service-wrapper">
	...
	<?php
		...
		if( !empty($room_services) ){
			foreach( $room_services as $room_id => $temp_services ){
				echo '<h3 class="gdlr-room-service-title" >' . get_the_title($room_id) . '</h3>';
				
				foreach( $temp_services as $service ){
					$active = false; //in_array($service, $selected_service);
					$option = json_decode(gdlr_decode_preventslashes(get_post_meta($service, 'post-option', true)), true);
					
					echo '<div class="gdlr-room-service-option">';
					echo '<input type="hidden" name="service-room[]" value="' . $room_id . '" />';
					if( !empty($option['service-type']) && $option['service-type'] == 'regular-service' && 
						!empty($option['always-enable']) && $option['always-enable'] == 'enable' ){
						
						echo '<label class="gdlr-room-service-checkbox gdlr-active">';
						echo '<input type="hidden" name="service-room-select[]" value="' . $service . '" />';
						echo '</label>';
					}else{
						echo '<label class="gdlr-room-service-checkbox ' . ($active? 'gdlr-active': '') . '">';
						echo '<input type="checkbox" name="service-room-select[]" value="' . $service . '" ' . ($active? 'checked': '') . ' />';
						echo '</label>';
					}
					echo '<span class="gdlr-room-service-title">' . get_the_title($service) . '</span>';
					
					if( $option['service-type'] == 'parking-service' && $option['car'] == 'car' ){
						echo '<input type="text" name="service-room-amount[]" value="1" ' . ($active? '': 'disabled') . ' />';
						echo '<span class="gdlr-room-service-title">';
						if( !empty($option['car-unit']) ){
							echo strtolower($option['car-unit']);
						}else{
						...
						echo '<input type="hidden" name="service-room-amount[]" value="1" ' . ($active? '': 'disabled') . ' />';
					}
	...
	<form class="gdlr-booking-contact-form" method="post" data-ajax="<?php echo AJAX_URL; ?>">
		...
		<?php
			if( !empty($hostel_option['enable-booking-term-and-condition']) && $hostel_option['enable-booking-term-and-condition'] == 'yes' &&
				...
				echo '<input type="checkbox" name="term-and-agreement" data-error="' . esc_html__('To continue, you must accept our terms & conditions', 'gdlr-hostel') . '" />';
				echo sprintf(__('I agree to <a href="%s" target="_blank" >terms & conditions</a>', 'gdlr-hostel'), $condition_page);
				echo '</div>';
			}
		?>
