	...
	*/
	...
	if( !function_exists('gdlrs_print_room_item') ){
		function gdlrs_print_room_item( $settings = array() ){
			...
			if( in_array($settings['room-style'], array('classic', 'modern', 'modern-new', 'medium', 'medium-new', 'image')) && !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
				if( empty($settings['carousel-navigation']) || $settings['carousel-navigation'] == 'default' ){
			...
			if( $settings['room-style'] == 'medium' || $settings['room-style'] == 'medium-new' ){
				...
				if( !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
					$ret .= gdlrs_get_medium_room_carousel($query, $settings['thumbnail-size'], $settings['room-style']);
				}else{
					$ret .= gdlrs_get_medium_room($query, $settings['thumbnail-size'], $settings['room-style']);
				}
				...
				if( !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
					$ret .= gdlrs_get_modern_new_room_carousel($query, $settings['room-size'], $settings['thumbnail-size']);
				}else{
					$ret .= gdlrs_get_modern_new_room($query, $settings['room-size'], $settings['thumbnail-size']);
				}
			}else if( $settings['room-style'] == 'image' ){
				if( !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
					$ret .= gdlrs_get_image_room_carousel($query, $settings['room-size'], $settings['thumbnail-size']);
				}else{
					$ret .= gdlrs_get_image_room($query, $settings['room-size'], $settings['thumbnail-size']);
				}
			}
			$ret .= '<div class="clear"></div>';
			$ret .= '</div>';
			...
			if( in_array($settings['room-style'], array('classic', 'modern', 'modern-new', 'medium', 'medium-new', 'image')) && !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
				if( !empty($settings['carousel-navigation']) && $settings['carousel-navigation'] == 'bottom' ){
					$ret .= '<div class="gdlr-room-bottom-nav gdlr-nav-container" >';
					$ret .= '<i class="icon-angle-left gdlr-flex-prev"></i>';
					if( !empty($settings['carousel-navigation-text']) && !empty($settings['carousel-navigation-link']) ){
						$ret .= '<a href="' . esc_attr($settings['carousel-navigation-link']) . '" class="gdlr-item-separator" >' . $settings['carousel-navigation-text'] . '</a>';
					}
	...
	if( !function_exists('gdlrs_get_medium_room') ){
		function gdlrs_get_medium_room($query, $thumbnail_size, $room_style = 'medium'){
			$ret  = '';
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<a class="gdlr-button with-border" href="' . get_permalink() . '">' . __('Check Details', 'gdlr-hostel') . '<i class="fa fa-long-arrow-right icon-long-arrow-right"></i></a>';
	...
	// get room style
	if( !function_exists('gdlrs_get_medium_room_carousel') ){
		function gdlrs_get_medium_room_carousel($query, $thumbnail_size, $room_style = 'medium'){
			$ret  = '';
	...
	if( !function_exists('gdlr_get_modern_room') ){
		function gdlr_get_modern_room($query, $size, $thumbnail_size){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<div class="gdlr-room-thumbnail">' . gdlr_get_room_thumbnail($post_option, $thumbnail_size) . '</div>';
				$ret .= '<h3 class="gdlr-room-title"><a href="' . get_permalink() . '" >' . get_the_title() . '</a></h3>';
				$ret .= '<a href="' . get_permalink() . '" class="gdlr-room-detail">' . __('Check Details', 'gdlr-hostel') . '<i class="fa fa-long-arrow-right icon-long-arrow-right"></i></a>';
				$ret .= '</div>'; // gdlr-ux
				$ret .= '</div>'; // gdlr-item
				$ret .= '</div>'; // gdlr-column-class
				$current_size ++;
	...
	if( !function_exists('gdlrs_get_modern_new_room') ){
		function gdlrs_get_modern_new_room($query, $size, $thumbnail_size){
			$current_size = 0; $ret  = '';
			while($query->have_posts()){ $query->the_post();
				$post_option = json_decode(gdlr_decode_preventslashes(get_post_meta(get_the_ID(), 'post-option', true)), true);
	...
	if( !function_exists('gdlrs_get_modern_new_room_carousel') ){
		function gdlrs_get_modern_new_room_carousel($query, $size, $thumbnail_size){
			$ret = ''; 
	...
	if( !function_exists('gdlrs_get_image_room') ){
		function gdlrs_get_image_room($query, $size, $thumbnail_size){
			$current_size = 0; $ret  = '';
			while($query->have_posts()){ $query->the_post();
				$post_option = json_decode(gdlr_decode_preventslashes(get_post_meta(get_the_ID(), 'post-option', true)), true);
				...
				$ret .= '<div class="' . gdlr_get_column_class('1/' . $size) . '">';
				$ret .= '<div class="gdlr-item gdlr-room-item gdlr-room-image">';
				$ret .= '<div class="gdlr-ux gdlr-room-image-ux">';
				
				$ret .= '<div class="gdlr-room-thumbnail-wrap">';
				$ret .= '<div class="gdlr-room-thumbnail-inner">' . gdlr_get_room_thumbnail($post_option, $thumbnail_size) . '</div>';
				$ret .= '<div class="gdlr-room-thumbnail-overlay" >';
				$ret .= '<div class="gdlr-room-title-wrap" >';
				if( !empty($post_option['room-image-top-info']) ){
					$ret .= '<div class="gdlr-room-image-top-info" >' . $post_option['room-image-top-info'] . '</div>';
				}
				$ret .= '<h3 class="gdlr-room-title"><a href="' . get_permalink() . '" >' . get_the_title() . '</a></h3>';
				if( !empty($post_option['room-image-bottom-info']) ){
					$ret .= '<div class="gdlr-room-image-bottom-info" >' . $post_option['room-image-bottom-info'] . '</div>';
				}
				$ret .= '</div>';
				$ret .= '</div>';
				$ret .= '</div>';
	...
	if( !function_exists('gdlrs_get_image_room_carousel') ){
		function gdlrs_get_image_room_carousel($query, $size, $thumbnail_size){
			$ret = ''; 
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<li class="gdlr-item gdlr-room-image">';
				$ret .= '<div class="gdlr-room-thumbnail-wrap">';
				$ret .= '<div class="gdlr-room-thumbnail-inner">' . gdlr_get_room_thumbnail($post_option, $thumbnail_size) . '</div>';
				$ret .= '<div class="gdlr-room-thumbnail-overlay" >';
				$ret .= '<div class="gdlr-room-title-wrap" >';
		...
		function gdlr_get_room_thumbnail($post_option, $size = 'full'){
			...
			switch($post_option[$type . 'thumbnail-type']){
				...
				case 'gallery': 
					...
					if( !empty($image_id) ){
						$image_list = array();
						if( !empty($post_option['thumbnail-slider']) ){
							$slides = json_decode($post_option['thumbnail-slider'], true);
							if( !empty($slides[0]) ){
						...
						$ret  = '<div class="gdlr-thumbnail-gallery" >';
						$ret .= '<a href="#" data-fc-gallery="' . esc_attr(json_encode($image_list)) . '" >';
						$ret .= gdlr_get_image($image_id, $size);
						$ret .= '<div class="gdlr-thumbnail-gallery-overlay" ></div>';
						$ret .= '<div class="gdlr-thumbnail-gallery-label gdlr-title-font" ><i class="fa fa-image"></i>' . __('View Gallery', 'gdlr-hostel') . '</div>';
						$ret .= '</a>';
						$ret .= '</div>';
					}
					break;					
	...
	if( !function_exists('gdlr_hostel_room_info') ){
		function gdlr_hostel_room_info( $post_option = array(), $list = array(), $wrapper = true, $style = 'classic-style' ){
			global $hostel_option, $theme_option;
				...
				switch( $slug ){
					...
					case 'price-break-down': 
						if( !empty($post_option['room-base-price']) ){
							...
							if( $slug == 'price' || empty($hostel_option['booking-price-display']) || $hostel_option['booking-price-display'] == 'start-from' ){
								...
								if( empty($theme_option['hotel-style']) || $theme_option['hotel-style'] != 'v4-style' ){
									$ret .= '<span class="gdlr-head">' . __('Start From', 'gdlr-hostel') . '</span>';
									$temp_price = gdlr_hostel_money_format($start_from_price);
								}else{
									$ret .= '<span class="gdlr-head">' . __('From', 'gdlr-hostel') . '</span>';
									$temp_price = gdlr_hostel_money_format($start_from_price, '', 0);
								}
							}else{
								$price_breakdown = get_hostel_price_breakdown_popup($post_option);
								$temp_price = gdlr_hostel_money_format($price_breakdown['total']);
							}
							$ret .= '<span class="gdlr-tail">';
							if( empty($theme_option['hotel-style']) || $theme_option['hotel-style'] != 'v4-style' ){
								if(!empty($post_option['room-type']) && $post_option['room-type'] == 'private'){
									$ret .= sprintf(__('%s / Room', 'gdlr-hostel'), $temp_price);
								}else{
									$ret .= sprintf(__('%s / Guest', 'gdlr-hostel'), $temp_price);
								}
							}else{
								$ret .= $temp_price;
							}
							...
							if( $slug == 'price-break-down' ){
								...
								$ret .= '<div class="gdlr-price-break-down" ><span>' . __('* view price breakdown', 'gdlr-hostel') . '</span>';
					...
					case 'breakfast-included':  
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Breakfast Included', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'room-service':   
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Room Service', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
