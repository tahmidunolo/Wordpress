<?php
	...
	*/
	...
	if( !function_exists('gdlrs_get_reservation_bar') ){
		function gdlrs_get_reservation_bar($single_form = false, $settings = array()){
			...
			if( empty($settings['enable-title']) || $settings['enable-title'] == 'enable' ){
				if( !$single_form ||
					empty($theme_option['hotel-style']) || $theme_option['hotel-style'] != 'v4-style' ||
					empty($theme_option['single-reservation-bar-title']) || $theme_option['single-reservation-bar-title'] == 'title' ){
					...
					if( !empty($post_option) ){
						...
						$ret .= '<div class="gdlr-reservation-bar-head-price" >';
						$ret .= '<span class="gdlr-head">' . __('From', 'gdlr-hotel') . '</span>';
						$ret .= '<span class="gdlr-tail">' . gdlr_hostel_money_format($start_from_price, '', 0) . '</span>';
						$ret .= '</div>';	
					}
				}
			}
		...
		function gdlrs_get_summary_form($data, $with_form = true, $coupon = ''){
			...
			if( !empty($data['service']) || !empty($data['service-room']) ){
				...
				if( !empty($data['service-room']) ){
					$room_services_price = gdlrs_calculate_room_service_price($data);
					foreach( $room_services_price as $room_id => $services ){
						if( $room == 'total' ) continue;

						$ret .= '<div class="gdlr-service-price-summary-head gdlr-sub-head" style="font-size: 13px;" >' . get_the_title($room_id) . '</div>';
					...
					$total_price += $room_services_price['total'];
				}
	...
	if( !function_exists('gdlrs_get_reservation_date_form') ){
		function gdlrs_get_reservation_date_form($single_form = false, $data = array()){
			...
			if( !empty($hostel_option['enable-checkin-time']) && $hostel_option['enable-checkin-time'] == 'yes' ){
				...
				$ret .= '<span class="gdlr-reservation-field-title" >' . __('Check In Time', 'gdlr-hostel') . '</span>';
				
				$ret .= '<span class="gdlr-resv-time-field gdlr-resv-time-hr" >';
				...
				$ret .= '<span class="gdlr-reservation-time-title" >' . __('Hour', 'gdlr-hostel') . '</span>';
				...
				$ret .= '<span class="gdlr-resv-time-field gdlr-resv-time-min" >';
				$checkin_min = empty($hostel_option['checkin-time-min'])? '00': $hostel_option['checkin-time-min'];
				$ret .= '<span class="gdlr-reservation-time-title" >' . __('Min', 'gdlr-hostel') . '</span>';
				$ret .= gdlr_get_reservation_combobox2(array(
