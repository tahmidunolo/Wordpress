<?php
	if( !function_exists('gdlr_hostel_summary_report') ){
		function gdlr_hostel_summary_report(){
...
<div class="gdlr-transaction-wrapper">
	<h3><?php _e('Summary Report', 'gdlr-hostel'); ?></h3>
	<form class="gdlr-transaction-form" method="GET" action="">
		<div class="gdlr-transaction-row">
			...
			<input type="hidden" name="page" value="hostel-summary-report" />
	...
	<form class="gdlr-transaction-table" method="post" >
		...
		<div class="transaction-filter">
			<span class="transaction-filter-title"><?php _e('Quick Filter :', 'gdlr-hostel'); ?></span>
			<?php
				$filters = array(
					'30days' => __('Show Last 30 Days', 'gdlr-hostel'),
					'7days' => __('Show Last 7 Days', 'gdlr-hostel'),
				);
		...
		<table>
			<tr>
				...
				<?php if( !empty($hostel_option['enable-hotel-branch']) && $hostel_option['enable-hotel-branch'] == 'enable' ){ ?>
					<th><?php _e('Branch', 'gdlr-hostel'); ?></th>
				<?php } ?>
				<th><?php _e('#Beds x Night', 'gdlr-hostel'); ?></th>
				<th><?php _e('Occupancy Rate', 'gdlr-hostel'); ?></th>
			</tr>
			...
			<?php
				...
				foreach( $rooms as $room_id => $occupy_times ){
					...
					if( $room_amount > 1 ){
						echo ' (' . $room_amount . ' ' . __('Beds', 'gdlr-hostel') . ')';
					} 
					echo '</td>';
					if( !empty($hostel_option['enable-hotel-branch']) && $hostel_option['enable-hotel-branch'] == 'enable' ){
						$room_terms = get_the_terms($room_id, 'hostel_room_category');
						echo '<td>';
						$count = 0;
