		...
		function gdlr_create_service_options(){
			...
			if( !empty($hotel_option['enable-hotel-branch']) && $hotel_option['enable-hotel-branch'] == 'enable' ){ 
				...
				if( post_type_exists('room') ){
					$branches = array_merge($branches, array(
						'hotel-room' => array(
							'title' => __('Assign to Hotel Room' , 'gdlr-hostel'),
							'type' => 'multi-combobox',
							'options' => gdlr_get_post_list('room', 'id'),
							'custom_field' => 'gdlr-room-id'
						),
					));
				}
			}
			...
			if( !empty($hostel_option['enable-hotel-branch']) && $hostel_option['enable-hotel-branch'] == 'enable' ){ 
				...
				if( post_type_exists('hostel_room') ){
					$branches = array_merge($branches, array(
						'hostel-room' => array(
							'title' => __('Assign to Hostel Room' , 'gdlr-hostel'),
							'type' => 'multi-combobox',
							'options' => gdlr_get_post_list('hostel_room', 'id'),
							'custom_field' => 'gdlr-hostel-room-id'
						),
					));
				}
			}
				...
				array(
					'page-layout' => array(
						...
						'options' => array_merge(array(
							...
							'always-enable' => array(
								'title' => __('Always Enable ( Customer will be unable to deselect this option )' , 'gdlr-hostel'),
								'type' => 'checkbox',
								'default' => 'disable',
							...
							'car-unit' => array(
								'title' => __('Car Unit ( Default To Cars )', 'gdlr-hostel'),
								'type' => 'text',
