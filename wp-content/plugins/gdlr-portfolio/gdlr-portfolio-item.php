<?php
	...
	*/
	...
	add_action('wp', 'gdlr_check_portfolio_single');
	if( !function_exists('gdlr_check_portfolio_single') ){
		function gdlr_check_portfolio_single(){
			global $gdlr_portfolio_single;
			$gdlr_portfolio_single = (get_post_type() == 'portfolio' && is_single());
		}
	}
	...
	if( !function_exists('gdlr_print_portfolio_item') ){
		function gdlr_print_portfolio_item( $settings = array() ){
			...
			if( $settings['portfolio-style'] == 'classic-portfolio' || 
				...
				global $gdlr_excerpt_length, $gdlr_excerpt_word; 
				$gdlr_excerpt_length = $settings['num-excerpt'];
				$gdlr_excerpt_word = __('View Project', 'gdlr-portfolio');
	...
	if( !function_exists('gdlr_get_portfolio_thumbnail_class') ){
		function gdlr_get_portfolio_thumbnail_class( $post_option ){
			global $gdlr_related_section, $gdlr_portfolio_single;
			if( $gdlr_portfolio_single && $post_option['inside-thumbnail-type'] != 'thumbnail-type'
			...
			switch($post_option[$type . 'thumbnail-type']){
				case 'feature-image':
					...
					if( !empty($image_id) ){
						...
						}else if( !$gdlr_portfolio_single || $gdlr_related_section ){
							$ret  = gdlr_get_image($image_id, $size);
							$ret .= '<span class="portfolio-overlay" >&nbsp;</span>';
							$ret .= '<div class="portfolio-overlay-content">';
							$ret .= '<a class="portfolio-overlay-wrapper" ' . gdlr_get_portfolio_thumbnail_link($post_option) . ' >';
				...
				case 'image':
					...
					break;
				case 'video': 
					if( $gdlr_portfolio_single && empty($gdlr_related_section) ){
