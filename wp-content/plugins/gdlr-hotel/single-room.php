				...
				<div class="with-sidebar-content <?php echo $gdlr_sidebar['center']; ?> columns">
					...
					?>">
						<div id="room-<?php the_ID(); ?>" <?php post_class(); ?>>
							<?php 
								$extra_class = '';
								if( empty($hotel_option['enable-single-booking-bar']) || $hotel_option['enable-single-booking-bar'] == 'enable' ){
									echo gdlr_get_reservation_bar(true); 
								}else{
								...
								<?php 
									if( empty($theme_option['hotel-single-style']) || $theme_option['hotel-single-style'] != 'blank-page' ){
										...
										<?php 
											...
											echo gdlr_hotel_room_info($gdlr_post_option, array(), true, $info_style); 
									}
								?>
