<?php
	...
	*/	
	...
	if( !function_exists('gdlr_calculate_room_service_price') ){
		function gdlr_calculate_room_service_price($data){
			$service_prices = array( 'total'=>0 );

			// get the necessary variable
			$day_diff = floor((strtotime($data['gdlr-check-out']) - strtotime($data['gdlr-check-in'])) / 86400);
			
			foreach( $data['service-room'] as $key => $room ){
				if( empty($data['service-room-select'][$key]) ){ continue; }

				$service = $data['service-room-select'][$key];			
				$service_prices[$room] = empty($service_prices[$room])? array(): $service_prices[$room];

				// get service
				$option = json_decode(gdlr_decode_preventslashes(get_post_meta($service, 'post-option', true)), true);
				$service_prices[$room][$service] = array(
					'title' => get_the_title($service),
					'price' => floatval($option['price'])
				);
				...
				if( $option['service-type'] == 'parking-service' ){
					if( $option['unit'] == 'night' ){
						$service_prices[$room][$service]['price'] *= $day_diff;
					}
					
					if( $option['car'] == 'car' ){
						$car_amount = empty($data['service-amount'][$key])? 1: $data['service-amount'][$key];
						$service_prices[$room][$service]['title'] .= '<span class="gdlr-sep">/</span>' . $car_amount . ' ';
						if( !empty($option['car-unit']) ){
							$service_prices[$room][$service]['title'] .= strtolower($option['car-unit']);
						}else{
							$service_prices[$room][$service]['title'] .= __('cars', 'gdlr-hotel');
						}
						$service_prices[$room][$service]['price'] *= $car_amount;
					}
				}else if( $option['service-type'] == 'regular-service' ){
					if( $option['unit'] == 'night' ){
						$service_prices[$room][$service]['price'] *= $day_diff;
					...
					if( $option['per'] == 'guest' ){
						$amount = 0;
						foreach( $data['gdlr-room-id'] as $room_id_key => $room_id ){
							if( $room_id == $room ){
								$amount = $amount + intval($data['gdlr-adult-number'][$room_id_key]);
								$amount = $amount + intval($data['gdlr-children-number'][$room_id_key]);
							} 
						}

						$service_prices[$room][$service]['title'] .= '<span class="gdlr-sep">/</span>' . $amount . ' ' . __('guests', 'gdlr-hotel') ;
						$service_prices[$room][$service]['price'] *= $amount;
					}else if( $option['per'] == 'room' ){
						$amount = 0;
						foreach( $data['gdlr-room-id'] as $room_id ){
							if( $room_id == $room ) $amount++;
						}

						$service_prices[$room][$service]['title'] .= '<span class="gdlr-sep">/</span>' . sizeOf($amount) . ' ' . __('room', 'gdlr-hotel') ;
						$service_prices[$room][$service]['price'] *= sizeOf($amount);
					}
				}
				
				$service_prices['total'] += $service_prices[$room][$service]['price'];
			}
			return $service_prices;
		}
	}
