<?php 
	...
	if( !function_exists('gdlr_hotel_set_block_dates') ){
		function gdlr_hotel_set_block_dates(){
			...
			if( !empty($hotel_option['block-date']) ){
				...
				delete_option('gdlr-hotel-block-dates');
			}
	...
	if( !class_exists('gdlr_hotel_ics') ){
		class gdlr_hotel_ics{
		    ...
		    function getIcsEventsAsArray($file) {
		    	...
		    	$icalString = gdlr_hotel_get_async_icals($file);
		    	if( !empty($icalString) ){
	...
	if( !function_exists('gdlr_hotel_set_async_icals') ){
		function gdlr_hotel_set_async_icals($files){
			global $gdlr_hotel_async_icals;

			$gdlr_hotel_async_icals = array();
			...
			if( !empty($responses) ){
				foreach( $responses as $response ){
					if( !empty($response->url) && !empty($response->body) ){
						$gdlr_hotel_async_icals[$response->url] = $response->body;
					}
				}
			}
		}
	}
	if( !function_exists('gdlr_hotel_get_async_icals') ){
		function gdlr_hotel_get_async_icals($file){
			global $gdlr_hotel_async_icals;

			if( !empty($gdlr_hotel_async_icals[$file]) ){
				return $gdlr_hotel_async_icals[$file];
			}
		...
		function gdlr_hotel_ical_routine(){
			...
			if( empty($timestamp) || $timestamp + $cache_time < $current_time ){
				...
				if( !empty($results) ){
					...
					gdlr_hotel_set_async_icals($files);

					foreach( $results as $result ){
						$updated = gdlr_hotel_set_ical($result->post_id, $result->meta_value) || $updated;
					} 
