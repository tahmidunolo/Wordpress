<?php
	...
	*/
	...
	if( !function_exists('gdlr_print_room_item') ){
		function gdlr_print_room_item( $settings = array() ){
			...
			if( $settings['room-style'] == 'medium' || $settings['room-style'] == 'medium-new' ){
				...
				if( !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
					$ret .= gdlr_get_image_room_carousel($query, $settings['room-size'], $settings['thumbnail-size']);
				}else{
					$ret .= gdlr_get_image_room($query, $settings['room-size'], $settings['thumbnail-size']);
				}
			}
			$ret .= '<div class="clear"></div>';
			$ret .= '</div>';
	...
	if( !function_exists('gdlr_get_image_room') ){
		function gdlr_get_image_room($query, $size, $thumbnail_size){
			$current_size = 0; $ret  = '';
			while($query->have_posts()){ $query->the_post();
				$post_option = json_decode(gdlr_decode_preventslashes(get_post_meta(get_the_ID(), 'post-option', true)), true);
	...
	if( !function_exists('gdlr_get_image_room_carousel') ){
		function gdlr_get_image_room_carousel($query, $size, $thumbnail_size){
			$ret = ''; 
			...
			switch($post_option[$type . 'thumbnail-type']){
				...
				case 'gallery':
					...
					if( !empty($image_id) ){
						...
						$ret .= '<div class="gdlr-thumbnail-gallery-label gdlr-title-font" ><i class="fa fa-image"></i>' . __('View Gallery', 'gdlr-hotel') . '</div>';
						$ret .= '</a>';
						$ret .= '</div>';
					}
					break;				
	...
	if( !function_exists('gdlr_hotel_room_info') ){
		function gdlr_hotel_room_info( $post_option = array(), $list = array(), $wrapper = true, $style = 'classic-style' ){
			global $hotel_option, $theme_option;
						...
						if( !empty($post_option['room-base-price']) ){
							...
							if( $slug == 'price' || empty($hotel_option['booking-price-display']) || $hotel_option['booking-price-display'] == 'start-from' ){
								...
								if( empty($theme_option['hotel-style']) || $theme_option['hotel-style'] != 'v4-style' ){
									...
									$ret .= '<span class="gdlr-tail">' . sprintf(__('%s / Night', 'gdlr-hotel'), gdlr_hotel_money_format($start_from_price)) . '</span>';
								}else{
									$ret .= '<span class="gdlr-head">' . __('From', 'gdlr-hotel') . '</span>';
									$ret .= '<span class="gdlr-tail">' . gdlr_hotel_money_format($start_from_price, '', 0) . '</span>';
								}
							}else{
							...
							if( $slug == 'price-break-down' ){
								...
								$ret .= '<div class="gdlr-price-break-down" ><span>' . __('* view price breakdown', 'gdlr-hotel') . '</span>';
