...
<?php
		...
		function gdlr_ajax_hotel_booking(){	
			...
			if( $_POST['state'] == 2 ){
				...
				if( empty($data['gdlr-check-in']) || empty($data['gdlr-check-out']) || $data['gdlr-check-out'] < $data['gdlr-check-in'] ){
					...
					$ret['content'] .= gdlr_get_booking_services($data['gdlr-hotel-branches'], $data['service'], $data['gdlr-room-id']);
	...
	if( !function_exists('gdlr_get_booking_room_query') ){
		function gdlr_get_booking_room_query($data, $room_number){
			...
			if( !empty($data['gdlr-hotel-branches']) ){
				$temp_args['tax_query'] = array(array(
					'taxonomy' => 'room_category',
					'field' => 'id',
					'terms' => intval($data['gdlr-hotel-branches'])
				));
			}
			$temp_rooms_query = new WP_Query($temp_args);
			$room_query = array();
			if( !empty($temp_rooms_query->posts) ){
				foreach( $temp_rooms_query->posts as $temp_room_query ){
					$temp_room = new stdClass;
					$temp_room->post_id = $temp_room_query->ID;
					$room_query[] = $temp_room;
				}
			}
	...
	if( !function_exists('gdlr_get_booking_services') ){
		function gdlr_get_booking_services($branches, $selected_service, $rooms = array()){
